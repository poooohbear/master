...
\renewcommand{\maketitle}{
    \begin{center}
        % \vspace*{2cm}
        {\Huge Master's thesis} \\[3.5cm]  % 修士論文
        {\huge Embedding of Tree Tensor Networks into Quantum Circuits of Two-Qubit Gates}\\[8mm]
        {\Large \begin{CJK}{UTF8}{ipxm}(二量子ビットゲート量子回路への\\ツリーテンソルネットワークの埋め込み)\end{CJK}}\\[8mm]
        {\Large January, 2025}\\[30mm]
        {\LARGE Supervisor: Prof. Synge Todo}\\[25mm]  % 指導教員名
        {\Huge 35236050 \quad Shota Sugawara}\\[2mm]
        {\textit{\large Department of Physics, Graduate School of Science, The University of Tokyo}\\}  % 所属
        \textit{\large 7-3-1 Hongo, Bunkyo-ku, Tokyo 113-0033, Japan.}\\
    \end{center}
    \thispagestyle{empty} 
}
...
% \renewcommand{\maketitle}{
% \thispagestyle{empty}
%     \setlength{\topmargin}{-10mm} % 必要な余白を手動で調整
%     \setlength{\oddsidemargin}{10mm} 
%     \setlength{\evensidemargin}{10mm}
%     \setlength{\textwidth}{150mm}
%     \setlength{\textheight}{220mm}
%     \begin{center}
%         {\Huge Master's thesis}\\[40mm]
%         {\huge Embedding of Tree Tensor Networks into Quantum Circuits of Two-Qubit Gates}\\[8mm]
%         {\Large \begin{CJK}{UTF8}{ipxm}(二量子ビットゲート量子回路への\\ツリーテンソルネットワークの埋め込み)\end{CJK}}\\[8mm]
%         {\Large January, 2025}\\[40mm]
%         {\LARGE Supervisor: Prof. Synge Todo}\\[25mm]
%         {\Huge 35236050 \quad Shota Sugawara}\\[2mm]
%         \textit{\large Department of Physics, Graduate School of Science, The University of Tokyo}\\
%         \textit{\large 7-3-1 Hongo, Bunkyo-ku, Tokyo 113-0033, Japan.}\\
%     \end{center}
...
% \begin{titlepage}
%     {%
%     \newgeometry{left=25mm,right=25mm,top=50mm,bottom=30mm}
%     \begin{center}
%         {\Huge Master's thesis}\\[40mm]
%         {\huge Quantum Error Mitigation in Hamiltonian Simulation \\ Using Quantum Singular Value Transformation}\\[8mm]
%         {\Large \begin{CJK}{UTF8}{ipxm}(量子特異値変換を用いたハミルトニアンシミュレーションにおける量子エラー抑制)\end{CJK}}\\[8mm]
%         {\Large January, 2025}\\[40mm]
%         {\LARGE Supervisor: Prof. Synge Todo}\\[25mm]
%         {\Huge 35236019 \quad Tokinori Oe}\\[2mm]
%         \textit{\large Department of Physics, Graduate School of Science, The University of Tokyo}\\
%         \textit{\large 7-3-1 Hongo, Bunkyo-ku, Tokyo 113-0033, Japan.}\\
%     \end{center}
%     \restoregeometry
...
Grant et al. proposed reducing circuit randomness by randomly selecting a subset of initial parameters and choosing the remaining parameters such that the entire circuit evaluates to the identity~\cite{init-strategy0}.
Volkoff and Coles introduced an initialization method that effectively reduces the dimensionality of the hyperparameter space by correlating the ansatz parameters~\cite{init-strategy1}.
Skolik et al. suggested an optimization technique that incrementally adds layers to the circuit, optimizing shallow circuits first rather than optimizing all parameters simultaneously~\cite{init-strategy2}.
...
Tensor networks are powerful mathematical frameworks that efficiently represent and manipulate high-dimensional data by decomposing tensors into interconnected lower-dimensional components~\cite{tn-review,tn-review2}.
They have been widely applied in various fields, including quantum many-body physics for approximating ground states of complex systems~\cite{dmrg,tebd,tn-review3}, machine learning for real data~\cite{MPSsupervised,MPSgenerative}, and combinatorial optimization problems.
...
Ran introduced a systematic decomposition method for an MPS into several layers of two-qubit gates with a linear next-neighbor topology~\cite{EncodingMPS}. 
...
\end{equation}
are hold, $L[U]^{(k)\dagger}$ can be considered a disentangler, transforming the quantum state into a product state.
...
Evenbly and Vidal proposed an iterative optimization technique that utilizes the calculation of environment tensors and SVD~\cite{env-tensor}.
Similarly, Shirakawa et al. employed this iterative optimization method to embed quantum states into quantum circuits~\cite{mpsoptim}.
...
We perform the SVD of the environment tensor and utilize the fact that the resulting unitary matrix serves as the optimal operator to increase the fidelity between the original tensor network and the constructed quantum circuit.
For a more detailed explanation of the optimization algorithm, please refer to \cite{mpsdecomp,mpsoptim} or the chapter on optimization algorithms for TTNs described later.


% decomposition of MPS
Rudolph et al. investigated the optimal sequence for combining these two methods to achieve the highest accuracy~\cite{mpsdecomp}.
...
We compute the SVD of $E$ and utilize the fact that the product $UV^\dagger$ is the unitary matrix that maximizes the magnitude of the inner product between the original TTN and the generated quantum circuit~\cite{mpsoptim}.
...
Rudolph et al. refers to this method as Iter[$D_i$, $O_{all}$]~\cite{mpsdecomp}, confirming that it achieves the highest accuracy regardless of the type of MPSs.
