...
I am profoundly thankful to Associate Professor Tsuyoshi Okubo.
...
Tokinori Oe, as a fellow lab member, always helped me through difficult times. Additionally, I had regular discussions with Hidemaro Suwa, Masahiko Yamada, Sayan Mukherjee, Atsushi Iwaki, Tatsuya Sakashita, Keiichi Tamai, Tohru Mashiko, Yuma Nakanishi, Rihito Sakurai, Shinichiro Akiyama, Xun Zhao, Hopai Kwok, Sora Shiratani, Soichiro Imamura, Ruixiao Cao, Kazushige Ueda, and Ruqing Xu during our meetings, and their broad knowledge of physics was very stimulating for me. 
...
\cleardoublepage
\begin{abstract}
    Variational Quantum Algorithms (VQAs) are being highlighted as key quantum algorithms for demonstrating quantum advantage on Noisy Intermediate-Scale Quantum (NISQ) devices, which are limited to executing shallow quantum circuits because of noise.
However, the barren plateau problem, where the gradient of the loss function becomes exponentially small with system size, hinders this goal.
...
Variational Quantum Algorithms~(VQAs) represent the foremost approach for achieving quantum advantage with the current generation of quantum computing technologies.
...
However, the currently available Noisy Intermediate-Scale Quantum~(NISQ) devices are incapable of executing most algorithms due to their limited number of qubits and susceptibility to noise~\cite{nisq}.

VQAs are hybrid methods where classical computers optimize parameters of quantum circuits's ansatz to minimize the cost function evaluated by quantum computers.
...
The barren plateau phenomenon refers to the challenge in VQAs where the gradient of the cost function decreases exponentially as the system size increases.
...
Tensor networks are originally developed to efficiently represent quantum many-body wave functions.
Any quantum circuit can be naturally regarded as a tensor networks~\cite{tn-qc}, and it is sometimes possible to simulate quantum computers with a practical amount of time using tensor networks~\cite{google-ibm}.
Moreover, in recent years, their utility has been recognized and applied across various fields such as machine learning~\cite{tnml} and language models~\cite{tnlanguage}

In this study, we focus particularly on Matrix Product States~(MPSs) and Tree Tensor Networks~(TTNs) among various structures of tensor networks.
...
Recently, these excellent algorithms have been applied to the field of machine learning, continuing the exploration of new possibilities~\cite{MPSsupervised,MPSgenerative}.
TTNs are tree-like structures of tensors.
The procedures used in DMRG and TEBD have been applied to TTNs~\cite{dmrg-ttn,tebd-ttn}.
Additionally, the distance between any pair of leaf nodes scales in logarithmic order in TTNs while in linear order in MPSs.
As connected correlation functions generally decay exponentially with path length within a tensor network, TTNs are better suited to capture longer-range correlations and represent two-dimensional objects than MPSs.
...
While numerical results have shown that this method can indeed avoid the barren plateau, there is a general challenge in mapping tensor network states into shallow quantum circuits.
...
In this paper, we propose a method to embed TTNs into shallow quantum circuits composed of only two-qubit gates as shown in Figure~\ref{fig:task-setting}.
...
Quantum computers operate based on principles entirely different from those of classical computers, which are currently widespread globally.
...
Whether quantum computers will eventually solve problems that classical computers cannot remains uncertain, but extensive research is actively being conducted by many scientists.

% 量子優位性を期待できる理由1: 量子コンピュータを古典でシミュレートすることの大変さ
This paper presents two reasons suggesting that quantum computers can perform tasks that are beyond the capabilities of classical computers.
The first reason is that classical algorithms capable of simulating quantum computers are still unknown.
...
This becomes fundamentally impossible as the number of qubits increases.
...
Despite decades of effort by physicists, we still cannot simulate a quantum computer using a classical computer.

% 量子優位性を期待できる理由2: 計算量理論の観点
% 量子優位性を期待できる理由3: 実際に古典より良い計算量で解くアルゴリズムが考案されていること
The second reason is that we are aware of specific problems that are difficult to solve with classical computers but can be efficiently addressed by quantum computers.
The most renowned example is Shor's algorithm~\cite{shor1994algorithms}, which can factorize very large composite numbers with remarkable speed.
...
While there are high expectations for the realization of quantum computers, achieving practical quantum computers is taking a long time.
...
Qubits need to strongly interact with each other for information processing.
Building a quantum system that meets both requirements is very challenging with current quantum technologies.
...
However, quantum error correction entails significant overhead.
This means that to protect quantum information, we need to use many more physical qubits than the logical qubits we want to safeguard.
This extra requirement of physical qubits increases the complexity and resources needed, making it challenging to build reliable quantum computers in the near future.

% NISQデバイスの概要(nisq4.1)
% NISQデバイスの特徴1: qubit数が少ないこと
% NISQデバイスの特徴2: ノイズが大きいこと
NISQ devices refer to Noisy Intermediate-Scale Quantum devices, which are widely recognized as the current state of quantum computing technology.
NISQ devices have two limitations compared to the ideal quantum computers anticipated in the future.
First, they have a limited number of qubits, typically in several hundreds.
Consequently, they cannot execute general quantum algorithms that require a large number of qubits.
...
Although NISQ devices are still in the developmental stages of quantum technology, they are garnering attention as a crucial step towards the more powerful quantum technologies that will be developed in the future.
...
However, due to the limited number of qubits in NISQ devices, error correction cannot be implemented.
...
This keeps the quantum circuit depth shallow, reducing noise, unlike quantum algorithms for the fault-tolerant era.
...
Subsequently, an ansatz is proposed, which is a quantum operation dependent on a set of continuous or discrete parameters $\theta$ that can be optimized. 
...
Similar to classical machine learning, the cost function maps the trainable parameters $\theta$ to real numbers. 
More abstractly, it creates a hypersurface usually called the cost landscape, where the optimizer's job is to navigate this landscape to find the global minima.
...
Specifically, the minimum of $C(\theta )$ should correspond to the solution of the problem, ensuring it is faithful.
...
Finally, it should be possible to efficiently optimize the parameters $\theta$, ensuring the cost function is trainable.
...
Conversely, some ansatz architectures are problem-agnostic, meaning they can be applied even in the absence of relevant information.

Here, we review several significant ansatzes. 
The first is the hardware-efficient ansatz. 
The hardware-efficient ansatz is a type of ansatz designed to reduce the circuit depth needed for implementation on specific quantum hardware.
...
The hybrid ansatzes delegate part of the ansatz to classical hardware, thereby reducing the complexity of quantum computations. By incorporating classical hardware, the computational burden on quantum systems is alleviated. 
...
\end{equation}
This involves minimizing the expectation value of $H$ over a trial state $\ket{\psi(\theta)}=U(\theta)\ket{\psi_0}$ for a given ansatz $U(\theta)$ and initial state $\ket{\psi_0}$.
...
% QMLの導入
Quantum machine learning~(QML) typically involves utilizing a quantum computer to discern patterns in quantum data, with the objective of making accurate predictions on unknown and unseen data~\cite{qml0}. 
Many QML tasks are formulated as the VQA, and various applications have been developed, including classifiers~\cite{qml1,qml2}, autoencoders~\cite{q-autoencoder}, generative models~\cite{q-generative,born-machine}, quantum Generative Adversarial Networks~(GANs)~\cite{q-gan}, and quantum neural network architectures~\cite{q-nn}.

% 手法の詳細
This paper focuses on generative models, which have garnered significant attention in recent years.
Generative modeling is an unsupervised statistical learning task aimed at learning a probability distribution that can generate a given dataset.
...
The goal is to learn $q(x)$ as the parameterized probability distribution $p_\theta(x)=|\bra{x}U(\theta)\ket{\psi_0}|^2$, obtained by applying $U(\theta)$ to an input state and measuring in the computational basis.
...
\cite{born-machine} introduced a variational framework for training quantum circuit Born machines, demonstrated with both classical and synthetic datasets, such as the bars-and-stripes dataset, and synthetic datasets related to the preparation of GHZ states and coherent thermal states.
...
Despite significant advancements in the field of VQAs, numerous challenges remain that must be addressed to sustain the potential for quantum speed-ups.
Particularly, the barren plateau phenomenon in cost-function landscapes has emerged as a critical bottleneck for VQAs.
When a cost function has a barren plateau, its partial derivatives diminish exponentially with system size, making the landscape nearly flat.
...
The analysis of barren plateau has been extended to include shallow random layered ansatzes, confirming that barren plateau phenomenon can indeed arise even in shallow circuits.
...
Numerical experiments have confirmed that the gradient magnitudes in VQAs remained stable, leading to the conclusion that the barren plateau problem was successfully avoided.
...
Next, we introduce two of the most renowned tensor network forms, known as Matrix Product States~(MPS) and Tree Tensor Networks~(TTN), and discuss their useful property called canonical form. 
Finally, we provide an overview of previous research on embedding MPS into quantum circuits.
...
In this master's thesis, we primarily address problems in quantum mechanics, assuming that the elements of tensors are complex numbers.
As you know, a $d$-dimensional vector is an element of $\mathbb C^d$.
A matrix with $n$ rows and $m$ columns is an element of $\mathbb C^{n \times m}$.
...
As illustrated in Figure~\ref{fig:tensor-example}, a scalar has no legs, a vector has one leg, a matrix has two legs, and a rank-4 tensor has four legs.

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{tensor-example.pdf}
...
Particularly, singular value decomposition stands out as a crucial tool for harnessing the power of tensor networks in data compression.
Then, we will explain the operations performed on multiple tensors: tensor product and contraction.
Since the primary benefit of tensor network notation lies in its ability for networks composed of multiple tensors, this will enable the full potential of tensor networks to be realized.
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{fig-trace.pdf}
    \caption{An graphical example of trace.
...
Figure~\ref{fig:trace} is a graphical example of trace operation in tensor network notation.
...
As shown in Figure~\ref{fig:trace-cyclic}, we can prove $\mathrm{Tr}(AB)=\mathrm{Tr}(BA)$ by simply sliding the tensor to change its position in the network.
Although this example is quite simple, the intuitive transformations of equations in tensor network notation become even more beneficial as the number of tensors in the network increases.

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{trace-cyclic.pdf}
    \caption{The diagram illustrates the cyclic property of the trace using tensor networks. In (a), the equation $\mathrm{Tr}(AB)$ is represented in tensor network notation. Tensor network notation allows for free transformations as long as the information about which axes are connected is preserved. Thus, it can be transformed from (a) to (b) to (c). Since (c) represents the equation $\mathrm{Tr}(BA)$ in tensor network notation, the cyclic property of the trace is thereby demonstrated.}
...
\begin{equation}
    I \coloneqq i_1 + d_1 i_2 + d_1 d_2 i_3 + \dots + d_1 d_{n-1} i_n ,
\end{equation}
where $d_x$ is the dimension of the $x$th index.
...
\begin{figure}
    ...
    By grouping the left axis in (a), we obtain (b). 
    Further grouping the central and right axes in (b) results in (c). 
    Since (c) represents a simple matrix product, the seemingly complex tensor network contraction is reduced to a matrix product. 
...
If we keep the top $k$ singular values in $S$ to form $S^{(k)}$, then $A^{(k)}=US^{(k)}V^\dagger$ is the closest rank-$k$ matrix to $A$ in the Frobenius norm.
...
By grouping $n$ indices as rows and $m$ indices as columns, and then performing SVD, we get a higher dimensional version of the SVD
\begin{equation}
    T_{i_1,\dots,i_m;j_1,\dots,j_n}=\sum_\alpha U_{i_1,\dots,i_m,\alpha}S_{\alpha,\alpha} V^\dagger_{j_1,\dots,j_n,\alpha},\end{equation}
where $U$ and $V$ are isometric ($U^\dagger U=V^\dagger V=I$ across the indicated partitioning, the conjugation in $V^\dagger$ is included to maintain consistency with conventional notation and is also applied with respect to this partitioning.
Similar to the matrix case, if $S^{(k)}$ is the $k\times k$ matrix formed by retaining the largest $k$ singular values in $S$, then
\begin{equation}
    T_{i_1,\dots,i_m;j_1,\dots,j_n}^{(k)}=\sum_\alpha^k U_{i_1,\dots,i_m,\alpha}S_{\alpha,\alpha}^{(k)} V^\dagger_{j_1,\dots,j_n,\alpha}
\end{equation}
...
Figure~\ref{fig:svd} represents the diagram of the SVD for tensors.
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{fig-svd.pdf}
    \caption{The diagram of the SVD for tensors.
    For a general tensor $A$, by grouping the left and right axes respectively, it can be regarded as a matrix as shown in (b). 
    Applying SVD to the matrix in (c) results in its decomposition into unitary matrices $U$ and $V$, and a diagonal matrix $S$ containing the singular values. 
    By performing splitting on (c), the SVD of the tensor is achieved while preserving the original tensor's axis information.}
    \label{fig:svd}
\end{figure}
...
\end{equation}
Diagrammatically, the tensor product is represented by placing two tensors adjacent to each other~\ref{fig:tensor-product}.
Therefore, the value of a network with disjoint tensors is simply the product of their values.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{tensor-product.pdf}
    \caption{The diagram of the tensor product for two tensors.
...
The most frequently utilized operation is contraction, which is a tensor product followed by a trace over the indices of two tensors.
...
The inner product of vectors, matrix-vector multiplications, and matrix-matrix multiplications are all forms of contraction~\ref{fig:contraction}.
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{fig-contraction.pdf}
...
The value for a given set of external indices is the product of the constituent tensor's values, summed over all internal index labeling consistent with the contractions.
...
You can contract tensor networks by starting with one tensor and sequentially contracting it with others.
...
% MPSの内積に関する例
As an example of how the order of contractions can significantly affect computational complexity, consider the bubbling of a ladder-shaped network that appears in calculations such as the inner product of  MPS.
One approach is to contract along the top of the ladder, then back along the bottom illustrated in Figure~\ref{fig:bubbling1}.
...
However, by contracting each rung in sequence, the tracked tensor's rank stays at three or below, leading to constant memory and linear time costs depicted in Figure~\ref{fig:bubbling2}.
\begin{figure}
    \centering
    ...
    However, the spatial computational complexity increases exponentially with the number of qubits at the intermediate point. 
    Consequently, for systems with a large number of qubits, current computers are unable to complete the contraction calculations.}
    \label{fig:bubbling1}
\end{figure}
\begin{figure}
    \centering
    ...
    For a ladder-shaped network, as in this example, sequentially contracting from the left achieves a computational complexity that is linear with respect to the number of qubits.}
    \label{fig:bubbling2}
\end{figure}



Although our example shows bubbling where one tensor contracts others one-by-one, this is not always the most efficient method; a multibubbling approach is often faster.
...
As illustrated in Figure~\ref{fig:mps}, Matrix Product States~(MPS) represent the simplest form of tensor networks, with tensors arranged in a single line. 
...
In contrast, Tree Tensor Networks~(TTN), depicted in Figure~\ref{fig:ttn}, have a more complex tree-like structure.
It is evident that MPS can be considered a subset of TTN.
...
In this paper, the key benefit is that at the canonical center, the local Singular Value Decomposition~(SVD) matches the global SVD, allowing for precise bond dimension reduction through truncation while appropriately moving the canonical center.
...
Unless otherwise specified, this paper assumes that any MPS and TTN are converted to the canonical form with $A^{(0)}$ as the canonical center.
...
% 取り組む問題を定式化しておく
We propose a method for embedding TTNs into shallow quantum circuits.
Any shape of TTN can be converted into a binary tree using SVD, so we assume a binary tree for simplicity.
However, this method can be generalized to embed TTNs of any shape.
...
We denote the original TTNs as $\ket{\psi_0}$, the $k$-th layer of the quantum circuit as $\mathrm{L}[U]^{(k)}$, the number of layers in resulting quantum circuit as $K$, and the resulting quantum circuit as $\prod_{k=1}^K \mathrm{L}[U]^{(k)}\ket{0}$.
Algorithm~\ref{algorithm:decomposition} details the analytical decomposition process for TTNs, as depicted in Figure~\ref{fig:decomposition-a}.
...
\begin{algorithm}
 \caption{Analytical decomposition.}
 \label{algorithm:decomposition}
 \KwIn{TTN$\ket{\psi_0}$,Maximum layers $K$}
...
Truncating bond dimension $\chi$ TTNs to bond dimension two TTNs can be done accurately, similar to MPSs, by shifting the canonical center so that the local SVD matches the global SVD.
...
As previously mentioned, naively transforming a general tensor network can require exponentially large memory relative to the number of qubits.
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{fig-decomposition-b.pdf}
    \caption{A diagram of a penetration algorithm. It contracts two connected tensors, reorders axes, and separates them using SVD, making it seem like their positions have swapped.
    }
    \label{fig:decomposition-b}
\end{figure}

To address the issue of tensor network transformations arising from the complexity of TTN structures, we employ a penetration algorithm as a submodule.
As illustrated in Figure~\ref{fig:decomposition-b}, the penetration algorithm operates by contracting two tensors connected by a single edge along the connected axis, appropriately reordering the axes, and then separating them using SVD.
This makes it appear as if the positions of the two tensors have swapped.
Additionally, by adjusting the number of singular values retained during the SVD, we can balance approximation accuracy and computational cost.
...
Algorithm~\ref{algorithm:transformation} details the transformation of tensor networks using the penetration algorithm, as depicted in Figure~\ref{fig:decomposition-c}.
\begin{figure}
    \centering
...
\end{figure}
We assign numbers to $\ket{\psi^{(k)}}$ in breadth-first search~(BFS) order from the root.
We denote $i$-th tensor of $\ket{\psi^{(k)}}$ as $A^{(i)}$.
...
For each tensor $\mathrm{L}[U]^{(k)\dagger}_i$ in $\mathrm{L}[U]^{(k)\dagger}$, first use SVD to split it into upper tensor $U_p$ and lower tensor $Lo$ to reduce the computational complexity in the penetration algorithm.
...
Consequently, despite minor approximation errors during penetration, $\mathrm{L}[U]^{(k)\dagger}\ket{\psi^{(k)}}$ can be transformed into a TTN structure.

\subsection{Combinations of analytic decomposition and optimization}
...
\end{algorithm}
The primary difference lies in the ansatz of the quantum circuits; however, the optimization algorithm can be executed in a similar manner.

Algorithm~\ref{algorithm:optimization} details the optimization process for TTNs, as depicted in Figure~\ref{fig:optimization}.
% \begin{figure*}[!h]
%   \centering
...
To address this, we compute the SVD of $E$ and utilize the fact that the product $UV^\dagger$ is the unitary matrix that minimizes the Frobenius distance to $E$.
...
\begin{algorithm}
 ...
 \KwIn{TTN$\psi_0$,Maximum layers $K$}
 \KwOut{Quantum Circuit $\prod_{k=1}^K \mathrm{L}[U]^{(k)}\ket{0}$}
...
For a TTN with a maximal bond dimension $\chi$, the memory requirements scale $\mathcal{O}(N\chi^3)$.
However, the computational complexity of transforming the TTN into its canonical form scale $\mathcal{O}(N\chi^4)$.
Most of the algorithms related to TTNs require transformation into canonical form.
...
In the penetration algorithm, the SVD of a matrix with row dimension $\chi^2$ and column dimension $\chi$ has a complexity of $\mathcal{O}(\chi^4)$.
...
The first state vector represents a uniform superposition over the binary data samples in the 4x4 bars and stripes (BAS) dataset, which has become a canonical benchmark for generative modeling tasks in quantum machine learning.
...
The Hamiltonian for this model is given by the following equation,
\begin{equation}
...
By varying the ratio of the first and second nearest-neighbor interactions, the $J_1$-$J_2$ Heisenberg model generates complex quantum many-body phenomena and has been widely studied.
In this paper, we utilized $J_2/J_1=0.5$.
Both state vectors are two-dimensional systems with long-range correlations, making them more suitably represented by TTNs rather than MPSs.
...
% We set the hyperparameters to 1000 optimization sweeps and a learning rate of 0.6.

To measure the accuracy of embedding into quantum circuits, we utilized infidelity as the evaluation metric.
The infidelity $I_f$ between two quantum states, $\ket{\Psi}$ and $\ket{\Phi}$, is expressed by
\begin{equation}
...
\end{figure}
 ...
 %  \caption{Infidelities between original TTNs and generated quantum circuits. The bond dimensions of TTNs are 16 and the number of optimization steps is 1000. The learning rate for optimization is set to 0.6.}
 %  \label{fig:result2}
 % \end{figure}
  \begin{figure}
    \centering
...
However, embedding TTN introduces greater approximation errors than MPS. 
...
This indicates that Iter[$D_i$, $O_{all}$] is the most effective embedding method, achieving high accuracy in multiple fields, including machine learning and physics, thus affirming the critical importance of the analytical decomposition algorithm.
...
In this study, we propose a method for embedding TTNs, which have a more complex structure than MPSs and can efficiently represent two-dimensional systems and systems with long-range correlations, into shallow quantum circuits composed solely of two-qubit gates.
We applied our proposed method to various types of TTNs and confirmed that it prepares quantum circuit parameters with better accuracy than embedding MPSs.
...
This study will serve as an important bridge for implementing hybrid algorithms combining tree tensor networks and quantum computing.
